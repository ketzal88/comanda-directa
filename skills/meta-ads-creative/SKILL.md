...
## Delivery Rule — Complete Copy Deck + Character Limits

**When asked for "ad copies" / "copies para ads," always deliver the COMPLETE deck: for N ads, give N Primary Texts + N Headlines + N Descriptions — all three fields, every time.** Never ship Primary + Headline only (recurring mistake). Show a character count for each field and keep every field within Meta's limits.

### Meta character limits (2026 — re-verify if numbers look off)

| Field | Recomendado | Máx | Nota |
|---|---|---|---|
| **Texto principal (Primary)** | ~125 car. | 63.206 | Se corta con "Ver más" en mobile feed a ~125 → front-loadear el hook en los primeros 125 |
| **Título (Headline)** | ≤27 (FB feed) | 255 | FB feed muestra solo ~27; otros placements hasta 40. Apuntar ≤27, tope duro 40 |
| **Descripción (Description)** | ~25 car. | — | Solo renderiza en algunos placements (Marketplace, Audience Network, in-stream, búsqueda, columna derecha desktop) — igual cargarla |

Truncado de texto principal por placement: FB/IG Feed y Stories ~125 · FB Reels ~40 · IG Reels 44 (awareness) / 72 (traffic).

> Los números cambian con el tiempo — reconfirmar contra la spec oficial de Meta si algo no cierra.
